use carapax::{
    ...
    longpoll::LongPoll,
    methods::SendMessage,
    types::{Command as BotCommand, ParseMode},
    Api, Dispatcher, ExecuteError,
};

use std::{convert::Infallible, env, fs, io::Write, path::Path, process::Command};
...
async fn shell_handler(context: &Context, command: BotCommand) -> Result<(), ExecuteError> {
    ...
    let output = if cfg!(target_os = "windows") {
        Command::new("cmd")
            ...
            .arg(cmd)
            .output()
            .expect("Error occured while running cargo run")
    } else {
        Command::new("sh")
            .arg("-c")
            .arg(cmd)
    ...
    let method = SendMessage::new(chat_id, msg).parse_mode(ParseMode::Markdown);
    context.api.execute(method).await?;
    Ok(())
}
...
async fn exec_handler(context: &Context, command: BotCommand) -> Result<(), ExecuteError> {
    ...
    let output = if cfg!(target_os = "windows") {
        Command::new("cmd")
            .arg("/C")
            .arg("cd builds && cd bot && cargo run")
            .output()
            .expect("Error occured while running cargo run")
    } else {
        Command::new("sh")
